...
namespace RentCar_Proyect_.UsuarioForms
{
    ...
    {
        UsuariosForm user;
        public CreateUserForm(UsuariosForm us)
        {
            InitializeComponent();
            this.user = us;
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (IsValid())
            {
                _context.Create(txtNombre.Text, txtUserName.Text, txtpass.Text, TiposUsuario.Text);
                this.Close();
                user.UserDataGrid.DataSource = _context.Getlist();

                MessageBox.Show("El Usuario agrego con exito", "Guardada", MessageBoxButtons.OK, MessageBoxIcon.Information);
